...
(define *words*
  (list
    ...
    (list :read-line (nmk-lambda (label)
                                 (when (string? label) (display label) (flush))
                                 (read-line)
                                 ))
    (list :int->str (nmk-lambda (int) (string (integer->char int))))
    (list :str->int (nmk-lambda (str) (char->integer (string-ref str 0))))
